...
void ADecoratorPatternCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	// set up gameplay key bindings
	PlayerInputComponent->BindAction("Jump", IE_Pressed, this, &ACharacter::Jump);
	PlayerInputComponent->BindAction("Jump", IE_Released, this, &ACharacter::StopJumping);
	PlayerInputComponent->BindAxis("MoveRight", this, &ADecoratorPatternCharacter::MoveRight);

	PlayerInputComponent->BindTouch(IE_Pressed, this, &ADecoratorPatternCharacter::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &ADecoratorPatternCharacter::TouchStopped);
}

void ADecoratorPatternCharacter::MoveRight(float Value)
{
	// add movement in that direction
	AddMovementInput(FVector(0.f,-1.f,0.f), Value);
}

void ADecoratorPatternCharacter::TouchStarted(const ETouchIndex::Type FingerIndex, const FVector Location)
{
	// jump on any touch
	Jump();
}

void ADecoratorPatternCharacter::TouchStopped(const ETouchIndex::Type FingerIndex, const FVector Location)
{
	StopJumping();
}
